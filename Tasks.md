...
- [ ] Set up Supabase account, create project, and acquire connection strings
...
- [ ] Implement admin whitelisting middleware
- [ ] Deploy Bot backend (Vercel serverless functions / Render)
...
- [ ] Backend: Validate `initData` hash using `TELEGRAM_BOT_TOKEN` for secure API requests.
- [ ] Backend: Extract accurate User ID and Group ID off the validated `initData` payload instead of mocked IDs.
- [ ] Backend: Set up robust error handling for missing group access.
